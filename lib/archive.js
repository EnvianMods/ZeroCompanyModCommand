...
async function extractArchive(archivePath, destDir, sevenZipPath) {
  ...
  if (ext === '.zip') {
    ...
    try {
      ...
      return;
    } catch (_) { /* tar unavailable or failed — try the JS extractor */ }
  ...
  throw new Error(`Unsupported archive type: ${ext}. Use .zip, .7z, .rar, or install a folder.`);
}

module.exports = { extractArchive, findSevenZip };
