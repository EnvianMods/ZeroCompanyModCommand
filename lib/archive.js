'use strict';
// Archive extraction: .zip natively (bsdtar / extract-zip), .7z/.rar via the
// 7-Zip CLI. Windows builds ship 7-Zip in tools/7-Zip (7z.exe + 7z.dll, see
// BUNDLED.txt) so no separate install is needed; a user-configured path or an
// installed copy still wins when present.

const fs = require('fs');
const path = require('path');
const { execFileSync } = require('child_process');
const extractZip = require('extract-zip');

// The copy shipped with the app (tools/ rides along as extraResources in
// packaged builds). Windows: 7z.exe; Linux: 7zz when a build bundles one.
function bundledSevenZip() {
  const roots = [
    path.join(__dirname, '..', 'tools', '7-Zip'),
    process.resourcesPath ? path.join(process.resourcesPath, 'tools', '7-Zip') : null,
  ].filter(Boolean);
  const exe = process.platform === 'win32' ? '7z.exe' : '7zz';
  for (const root of roots) {
    const p = path.join(root, exe);
    try { if (fs.existsSync(p)) return p; } catch (_) {}
  }
  return null;
}

function findSevenZip(configured) {
  const candidates = [
    configured,
    bundledSevenZip(),
    'C:\\Program Files\\7-Zip\\7z.exe',
    'C:\\Program Files (x86)\\7-Zip\\7z.exe',
    // Linux (p7zip / 7-Zip official): whichever variant the distro ships.
    '/usr/bin/7z', '/usr/bin/7za', '/usr/bin/7zz', '/usr/local/bin/7zz',
  ].filter(Boolean);
  for (const c of candidates) {
    try { if (fs.existsSync(c)) return c; } catch (_) {}
  }
  // PATH lookup
  for (const name of ['7z', '7za', '7zz']) {
    try {
      execFileSync(name, ['--help'], { stdio: 'ignore' });
      return name;
    } catch (_) {}
  }
  return null;
}

// Post-extraction safety pass: symlinks/junctions are removed (a link inside an
// archive can point anywhere on disk), and every entry must resolve inside the
// destination — an archive that slips a file outside it is rejected outright.
function validateExtraction(destDir) {
  const rootReal = fs.realpathSync(destDir);
  const stack = [destDir];
  while (stack.length) {
    const dir = stack.pop();
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      const abs = path.join(dir, entry.name);
      if (entry.isSymbolicLink()) {
        fs.rmSync(abs, { recursive: true, force: true });
        continue;
      }
      const real = fs.realpathSync(abs);
      if (real !== rootReal && !real.startsWith(rootReal + path.sep)) {
        throw new Error(`The archive tried to place "${entry.name}" outside the extraction folder — install refused.`);
      }
      if (entry.isDirectory()) stack.push(abs);
    }
  }
}

// tar binaries that can read zips: Windows ships bsdtar as System32\tar.exe —
// use that absolute path first, because a bare `tar` on PATH may be GNU tar
// (Git for Windows, Linux) which cannot open zips at all.
function zipCapableTars() {
  const list = [];
  if (process.platform === 'win32') {
    const sysRoot = process.env.SystemRoot || process.env.windir || 'C:\\Windows';
    list.push(path.join(sysRoot, 'System32', 'tar.exe'));
  }
  list.push('tar');
  return list;
}

async function extractArchive(archivePath, destDir, sevenZipPath) {
  const ext = path.extname(archivePath).toLowerCase();
  fs.mkdirSync(destDir, { recursive: true });
  if (ext === '.zip') {
    // Windows' built-in bsdtar handles zips extract-zip chokes on (it can hang
    // forever on some archives, and it rejects the "./" directory entry that
    // bsdtar-built zips — e.g. Zero Company Mod SDK packages — carry). Order:
    // bsdtar → extract-zip → 7-Zip when present.
    for (const tar of zipCapableTars()) {
      try {
        execFileSync(tar, ['-xf', archivePath, '-C', destDir], { stdio: 'ignore', timeout: 120000 });
        validateExtraction(destDir);
        return;
      } catch (err) {
        if (/outside the extraction folder/.test(err.message)) throw err;
        /* this tar unavailable or failed — next candidate */
      }
    }
    try {
      await extractZip(archivePath, { dir: destDir });
      validateExtraction(destDir);
      return;
    } catch (err) {
      if (/outside the extraction folder/.test(err.message)) throw err;
      const sz = findSevenZip(sevenZipPath);
      if (!sz) throw err;
      fs.rmSync(destDir, { recursive: true, force: true });
      fs.mkdirSync(destDir, { recursive: true });
      execFileSync(sz, ['x', '-y', `-o${destDir}`, archivePath], { stdio: 'ignore', timeout: 120000 });
      validateExtraction(destDir);
      return;
    }
  }
  if (ext === '.7z' || ext === '.rar') {
    const sz = findSevenZip(sevenZipPath);
    if (!sz) {
      throw new Error(
        `${ext} archives need 7-Zip. Install 7-Zip or set its path in Settings, or extract the archive yourself and install the folder.`
      );
    }
    execFileSync(sz, ['x', '-y', `-o${destDir}`, archivePath], { stdio: 'ignore' });
    validateExtraction(destDir);
    return;
  }
  throw new Error(`Unsupported archive type "${ext}" (file: ${path.basename(archivePath)}). Use .zip, .7z, .rar, or install a folder.`);
}

module.exports = { extractArchive, findSevenZip, bundledSevenZip, validateExtraction };
