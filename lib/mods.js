...
class ModEngine {
  ...
  _installFromFolder(root, { fallbackName, sourceArchive, metaOverride, classified, origin, version, reuseId }) {
  ...
  zcsdkStatus() {
    ...
    const base = { installed: false, healthy: false, active: false, updateAvailable: false, parts: {}, neededBy, bundled: bundledInfo, available };
    ...
    const updateAvailable = installed && !!pkg && (behind(bridge.version, pkg.bridge) || behind(loader.version, pkg.loader));
  ...
  // ---------------------------------------------------------------- UE4SS runtime vault
  // GitHub keeps only ONE rolling experimental UE4SS build, and the stable
  // 3.0.x zips use a flat layout this manager cannot deploy — so "an older
  // UE4SS for my frozen game" can only come from builds this app installed
  // before. Every runtime install snapshots the current runtime first:
  // Binaries\Win64\dwmapi.dll + ue4ss\* minus the user's ue4ss\Mods and logs.
  _ue4ssRuntimeFiles() {
    const win64 = this.gameAbs(WIN64_REL);
    const out = [];
    if (fs.existsSync(path.join(win64, 'dwmapi.dll'))) out.push('dwmapi.dll');
    const ue4ssDir = path.join(win64, 'ue4ss');
    if (fs.existsSync(ue4ssDir)) {
      for (const rel of walkFiles(ue4ssDir)) {
        const lower = rel.toLowerCase();
        if (lower.startsWith('mods' + path.sep) || lower.endsWith('.log')) continue;
        out.push(path.join('ue4ss', rel));
      }
    }
    return out;
  }
  ...
  ue4ssSnapshot(label, meta) {
    try {
      if (!this.gamePath() || !this.ue4ssStatus().installed) return null;
      const files = this._ue4ssRuntimeFiles();
      if (!files.length) return null;
      const win64 = this.gameAbs(WIN64_REL);
      const vaultDir = this.store.modVaultDir(UE4SS_VAULT_KEY);
      const stamp = new Date().toISOString().replace(/[:.]/g, '-');
      const entryId = `${stamp}__${safeName(label || 'unknown-build')}`;
      const filesDir = path.join(vaultDir, entryId, 'files');
      fs.mkdirSync(filesDir, { recursive: true });
      for (const rel of files) {
      ...
      fs.writeFileSync(path.join(vaultDir, entryId, 'vault.json'), JSON.stringify({
        ...
        files: files.length, savedAt: new Date().toISOString(),
      }, null, 2));
      const entries = fs.readdirSync(vaultDir).sort().reverse();
      for (const stale of entries.slice(5)) fs.rmSync(path.join(vaultDir, stale), { recursive: true, force: true });
      return entryId;
    } catch (_) { return null; }
  }
    ...
    try {
      for (const entryId of fs.readdirSync(vaultDir).sort().reverse()) {
        try {
          ...
          entries.push({ entryId, label: m.label, tag: m.tag, name: m.name, asset: m.asset, publishedAt: m.publishedAt, savedAt: m.savedAt, files: m.files });
        } catch (_) {}
      }
    } catch (_) {}
    return entries;
  ...
  ue4ssRestore(entryId, currentLabel, currentMeta) {
    ...
    if (!files.length) throw new Error('That kept build is empty.');
    const win64 = this.gameAbs(WIN64_REL);
    if (!fs.existsSync(win64)) throw new Error('Game Win64 folder not found.');
    this.ue4ssSnapshot(currentLabel, currentMeta);
    for (const rel of this._ue4ssRuntimeFiles()) { try { fs.rmSync(path.join(win64, rel), { force: true }); } catch (_) {} }
    for (const rel of files) {
      const dst = path.join(win64, rel);
      fs.mkdirSync(path.dirname(dst), { recursive: true });
      fs.copyFileSync(path.join(filesDir, rel), dst);
    }
    fs.mkdirSync(path.join(win64, 'ue4ss', 'Mods'), { recursive: true });
    return manifest;
  }
...
module.exports = { ModEngine, classifyFolder, zcsdkMeta, isSidecar, compareVersions, MODS_REL, LOGIC_MODS_REL, WIN64_REL, UE4SS_MODS_REL };
